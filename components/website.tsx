...
import { business, homePath, galleryPath, path, imagePath, heroImage, closetImage, logoImage, type Language, type Gallery } from '@/lib/site';
import { alternatePath } from '@/lib/routes';
import { ProjectImage } from './project-image';
import { Survey } from './survey';

export function Header({lang,kind}: {lang:Language;kind:string}) {
 const c=content[lang];
 const home=path(homePath(lang));
...
export function Footer({lang}: {lang:Language}) {
 ...
 return <>
  <footer className="wrap footer">
   <div className="footer-top"><div><a href={path(homePath(lang))}><img className="brand" src={imagePath(logoImage,720)} alt={business.name} width="180" height="72" loading="lazy"/></a><p>{c.footer}</p></div>
    <div className="footer-nav"><a href={path(galleryPath(lang,'kitchens'))}>{c.kitchen}</a><a href={path(galleryPath(lang,'closets'))}>{c.closet}</a><a href={path(homePath(lang))+'#contact'}>{c.quote}</a></div>
   ...
   <div className="footer-bottom"><span>© {new Date().getFullYear()} {business.name}. {c.allRights}</span><a href={path(alternatePath(lang,'privacy'))}>{c.privacy}</a></div>
  </footer>
  <div className="mobile-contact"><a href={'tel:'+business.telephone} data-track="phone_click"><Phone size={17}/>{c.call}</a><a href={path(homePath(lang))+'#contact'} data-track="quote_engagement">{c.quote}<ArrowUpRight size={18}/></a></div>
 </>;
}
 ...
 return <>
  <section className="hero wrap">
   ...
   <div className="hero-visual">
    ...
    <a className="image-caption" href={path(galleryPath(lang,'kitchens'))}><span>{c.photoLabel}</span><ArrowUpRight size={22}/></a>
  ...
  <section className="section wrap" id="work"><span className="legacy-anchor" id="past-projects" aria-hidden="true"/>
   ...
   <div className="project-grid">{(['kitchens','closets'] as Gallery[]).map((g,i)=><a className="project-card" href={path(galleryPath(lang,g))} key={g}>
...
export function Privacy({lang}: {lang:Language}) {
 const c=content[lang];
 return <section className="wrap section prose"><a className="text-link" href={path(homePath(lang))}>← {c.back}</a><h1>{c.privacy}</h1><p>{c.privacyIntro}</p><p>{c.privacySurvey}</p><p>{c.privacyAnalytics}</p><p>{c.privacyContact} <a href={'mailto:'+business.email}>{business.email}</a>.</p><p>{lang==='en'?'Hosting providers may collect technical access logs for security.':'Los proveedores de alojamiento pueden recopilar registros técnicos de acceso por motivos de seguridad.'}</p></section>;
}
